...
SHORTEN = {
    ...
    'tool · read_file result (artifacts_test.py again)': 'tool · read_file result (same file again)',
...
parts = [
    ...
    f"aria-label='Per-message sizes of an 18-message coding agent transcript. "
    f"File bodies entering through tool results and repeating through tool arguments dwarf all prose.'>",
]
...
for color, label, est in [
    ...
    (GREEN, 'file/log bytes in tool results', 'file/log bytes in tool results'),
...
stages = [
    ...
    ('offload', 'large tool outputs offloaded to artifacts', 'results become pointers; old write_file arguments remain'),
    ('clip', 'plus old tool arguments clipped', 'recent 6 messages untouched; everything else is stubs'),
]
...
parts = [
    ...
    f"48.7k chars raw, 18.8k after offloading tool outputs, 2.5k after also clipping old tool arguments.'>",
]
